...
const String _projectsPath = 'projects';
...
Stream<(bool, List<Project>?)> listenToAllProjects() {
  return FirebaseRTDBService.instance.listenToPath(_projectsPath).map((event) {
    try {
...
Future<(bool, List<Project>?)> readAllProjects() async {
  try {
    final snapshot = await FirebaseRTDBService.instance.readPath(_projectsPath);
    ...
    final projects = data.entries
        .map(
            (entry) => Project.fromJson(Map<String, dynamic>.from(entry.value)))
        .toList();
    return (true, projects);
  } on FirebaseException catch (e) {
